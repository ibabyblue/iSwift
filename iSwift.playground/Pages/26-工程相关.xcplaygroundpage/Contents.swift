...
var c = Cat()
c.test1() //消息发送,如果单独使用"@objc"依然是使用虚表。
c.test2() //虚表
...
//关联对象的本质就是键值对，但是需要我们自己绑定一个编译期已知地址值（静态存储属性）。
//由于仅需要地址绑定，所以为了内存空间考虑，静态存储属性使用Bool类型或者Void?类型最好（仅占用1个字节）
