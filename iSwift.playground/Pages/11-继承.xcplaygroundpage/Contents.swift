...
/*
 • 实例属性：
 - 子类只能重写父类 var 属性，不能重写 let 属性，因为let属性初始化后不能再改变，所以子类重写let属性无意义。
 - 重写时，属性名称与类型需一致
 - 子类只能将父类的存储、计算属性，重写为计算属性，不能重写为存储属性（如果是存储属性会破坏内存布局）。
    - 内存布局的确定性：存储属性需要固定的内存布局，子类不能改变这一点。
    - 计算属性不影响内存：计算属性只是动态计算值，不会改变内存分配或布局。
    - 继承链中的一致性：重写为存储属性会破坏父类和子类之间的内存和行为一致性。
    - 多态性和动态调度的要求：存储属性在继承中的多态性行为可能引发问题，而计算属性可以动态处理这种情况。
